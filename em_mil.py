...
from util.system_related import get_exact_file_name_from_path
...
def initialize(li_mpp, li_fn_slide, len_tile_pxl, stride_factor):
    n_res = len(li_mpp)
    n_slide = len(li_fn_slide)
    li_im_tissue_int = n_slide * [None]
    li_tissue_mpp = n_slide * [None]    # The actual mpp of test slides
    li_model, li_li_im_discriminative, li_im_count, li_li_tile_pos = \
        n_res * [None], [], [], []

    for iS, fn_slide in enumerate(li_fn_slide):
        slaid = get_slide(fn_slide)
        mpp_x, mpp_y, is_from_mpp_xy = get_mpp_slide(slaid)
        li_tissue_mpp[iS] = 0.5 * (mpp_x + mpp_y)
    # 각 레졸루션에 대해
